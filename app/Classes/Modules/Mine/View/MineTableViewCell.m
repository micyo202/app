...
- (instancetype) initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier{
    if (self = [super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        ...
        [self.contentView addSubview:self.cSwitch];
...
+ (CGFloat) getHeightForText:(MineModelItem *)item{
    ...
    float defaultH = 50.0f, buttonH = 50.0f, subH = 80.0f;
...
- (UIButton *) cButton{
    if (_cButton == nil) {
        ...
        // 注册按钮点击事件
        [_cButton addTarget:self action:@selector(buttonAction:) forControlEvents:UIControlEventTouchUpInside];
    }
    return _cButton;
}
